...
const updateVault = () => {
	$.post("http://vrp_homes/requestVault",JSON.stringify({}),(data) => {
		...
		$('#inventory').html(`
			<div class="peso"><b>BUSY:</b>  ${(data.peso).toFixed(2)}    <s>|</s>    <b>AVAILABLE:</b>  ${(data.maxpeso-data.peso).toFixed(2)}    <s>|</s>    <b>SIZE:</b>  ${(data.maxpeso).toFixed(2)}</div>
			<div class="peso2"><b>BUSY:</b>  ${(data.peso2).toFixed(2)}    <s>|</s>    <b>AVAILABLE:</b>  ${(data.maxpeso2-data.peso2).toFixed(2)}    <s>|</s>    <b>SIZE:</b>  ${(data.maxpeso2).toFixed(2)}</div>
			<div class="esquerda">
				${nameList2.map((item) => (`
					<div class="item" style="background-image: url('images/${item.index}.png');" data-item-key="${item.key}">
			...
			<div class="meio">
				<input id="amount" class="qtd" maxlength="9" spellcheck="false" value="" placeholder="AMOUNT">
			</div>

			<div class="direita">
				${nameList.map((item) => (`
					<div class="item2" style="background-image: url('images/${item.index}.png');" data-item-key="${item.key}">
						<div id="peso">${(item.peso*item.amount).toFixed(2)}</div>
						<div id="quantity">${formatarNumero(item.amount)}x</div>
						<div id="itemname">${item.name}</div>
					</div>
